...
- DNA-inclusive：新增 Gate NA（`science-v1` reviewed regression；基于 `test/science_dna_cases.json` 冻结 `test/golden_na/**`；不要求 legacy C 具备对 DNA case 的分析能力），避免含 DNA 链结构成为“无回归覆盖的盲区”。
...
- Gate NA（DNA-inclusive，science-v1 reviewed regression）：`bash test_phase4_gate_na.sh`
- NA Golden：`test/golden_na/manifest.json`（`science-v1` reviewed freeze；包含 `core` + `.out/.xml/.ps/.wrl`，其中 `core` 带 DNA-aware `residues[]`）
