...
echo "== build rust staticlib (legacy-ffi) [release] ==" >&2
bash "${REPO_ROOT}/tools/cargo_sysroot.sh" build --manifest-path "${REPO_ROOT}/rust/Cargo.toml" --lib --features legacy-ffi --release

RUST_LIB="${REPO_ROOT}/rust/target/release/librnaview_hotcore.a"
if [[ ! -f "${RUST_LIB}" ]]; then
  echo "missing rust staticlib: ${RUST_LIB}" >&2
  exit 2
fi

OBJ_DIR="${REPO_ROOT}/obj/rustcore_release"
BIN_OUT="${REPO_ROOT}/bin/rnaview_rustcore_release"
